...
void read_from_file()
{
    ...
    //sscanf(h, "%s %d %d %s %s %s\n", temp.user, temp.year, temp.md, temp.start, temp.end, temp.name)
  ...
  /*  int fd;
    struct event *myPtr;
    struct event myInput;

    char buf[MAX_BUF];

    events.clear();

    fd = open(MY_DATA_FILE, O_RDONLY);
    while(read(fd, &buf[0], sizeof(myInput))>0)
    {
        myPtr = (struct event*)&buf[0];
        memcpy(&myInput, myPtr, sizeof(myInput));
        add_event(myInput);
    }
    close(fd); */
